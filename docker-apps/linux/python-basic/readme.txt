...
# ***** build and store a local copy of image including a tag with my own name. The n4demo refers to the Docker Hub image repository that we will push this to.
# *****  -t refers to tag and the . (dot) tells Docker where to find the files to build (current directory)
docker image build -t n4demo/python-basic:my-name-here .
...
docker build -t n4demo/python-basic:v1.0.0 -t n4demo/python-basic:latest .

# run the local image in a Linux VM hosted on your laptop inside Docker desktop
docker container run --name python_basic --rm -i -t n4demo/python-basic:my-name-here

# show all containers running or not. Is it still running?
docker ps -a

## show logs for this container - why did it fail?
docker logs -f n4demo/python-basic:my-name-here

#login to DockerHub using password: <password>
docker login --username n4demo -p <password>

# push the image to docker hub
docker push n4demo/python-basic:my-name-here

# now view in DockerHub in browser by signing in as: n4demo  password: <password>
