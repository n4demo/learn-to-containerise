...
# create the local image. n4demo refers to the repository, python-standard refers to the application and my-name-here is the tag for this application 
docker image build -t n4demo/python-standard:my-name-here .

# add 3 additional tags to the same image by rebuilding image. Note we add a version.
docker build -t n4demo/python-standard:v1.0.0 -t n4demo/python-standard -t n4demo/python-standard:latest .

# run the local image in a container hosted in a Linux VM hosted on your laptop
docker container run --name python-web-app --rm -i -t -p 5001:5000 n4demo/python-standard:my-name-here

# docker container run --name python-web-app --rm -i -t -p 5001:5000 n4demo/python-standard:latest
...
docker push n4demo/python-standard:v1.0.0

docker push n4demo/python-standard:latest
...
# Select Container Settings - edit the image to n4demo/python-standard:v1.0.0 image in Docker Hub
...
# now deploy the n4demo/python-standard:v1.0.0 web server image from Docker Hub into your kubernetes cluster

n4demo/python-standard:v1.0.0

# deploy python in a pod in kubernetes
kubectl run  python --image=n4demo/python-standard:latest  
